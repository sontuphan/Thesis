...
\subsection{Máy chủ nhãn thời gian}
...
\end{table}\\
(Tính đa dạng là biểu diễn của nhiều loại tài sản giao dịch khác nhau mà sàn 
giao dịch hỗ trợ, tính thanh khoản tham khảo mục 3.3.2)\\\\
Vì Poloniex là một sàn lớn và cung cấp đầy đủ các API cho việc thu thập dữ liệu, 
cũng như là lịch sử giao dịch với số lượng lớn nên Poloniex được chọn là sàn 
giao dịch đặc trưng dùng để phân tích trung luận văn này.
\subsection{Giao dịch (trên sàn giao dịch)}
...
nhất.\\\\
Trên mỗi giao dịch, Poloniex tính phí dựa theo vai trò người dùng trung giao 
